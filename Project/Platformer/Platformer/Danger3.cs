...
namespace Platformer
{
    ...
    class Danger3
    {
        ...
        public Vector2 Position
        {
            get
            {
                return basePosition + new Vector2(bounce, 0.0f);
            }
        }
        ...
        /// </summary>
        public Danger3(Level level, Vector2 position)
        {
            this.level = level;
        ...
        {
            texture = Level.Content.Load<Texture2D>("Sprites/spikeball1");
            origin = new Vector2(texture.Width / 2.0f, texture.Height / 2.0f);
            collectedSound = Level.Content.Load<SoundEffect>("Sounds/GemCollected");
        }
        public void LoadContent2()
        {
            texture = Level.Content.Load<Texture2D>("Sprites/spikeball2");
            origin = new Vector2(texture.Width / 2.0f, texture.Height / 2.0f);
            collectedSound = Level.Content.Load<SoundEffect>("Sounds/GemCollected");
        }
        ...
        public void Update(GameTime gameTime)
        {
            switch (PlatformerGame.currentGameState)
            {
                case PlatformerGame.GameState.Playing:
                    ...
                    const float BounceWidth = 2.10f;
                    const float BounceRate = 1.0f;
                    const float BounceSync = -0.75f;

                    // Bounce along a sine curve over time.
                    // Include the X coordinate so that neighboring gems bounce in a nice wave pattern.            
                    double t = timer * BounceRate + Position.Y * BounceSync;
                    bounce = (float)Math.Sin(t) * BounceWidth * texture.Width;
                    break;
            }
        }
